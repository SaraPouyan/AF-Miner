...
public class FUFMiner extends Classifier {
    //
    ...
    public double[][] getRule(List<double[]> dataList3, final double classId2){
        ...
        FUFSA fufsa = new FUFSA(dim * 2, random)  {
            @Override
...
abstract class FUFSA {
    ...
    private double imprRatio;
    private double step;
    private double visual;
    private double[] bestAf;
    Random random;

    public FUFSA(int dim, Random random) {
        //
        dimNumber      = dim;
    ...
    public double[][] run() {
        String fileName = "src/weka/classifiers/custom/FUF.fcl";
        FIS fis         = FIS.load(fileName, true);
        tempPopulation  = population.clone();
        ...
        for (int i = 0; i < maxIteration; i++) {
            ...
            for (int j = 0; j < populationNumber; j++){
                if(getFitness(population[j]) > getFitness(tempPopulation[j])){
                    Num++;

                }
                tempPopulation[j] = population[j].clone();
            }

            imprRatio =(double) Num/populationNumber;
            fis.setVariable("itrNum", Itrnum);
            fis.setVariable("improvementRatio", imprRatio );
            fis.evaluate();



            double CW = fis.getVariable("Weight").getValue();
